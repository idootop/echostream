...
- [x] **流协议重设计**：三帧协议 StreamOpen(metadata 协商) / StreamMsg(去 name 按 id 路由，核心仅传输语义)
      / StreamEnd(code+message+trailers)；核心不做上层语义（采样时钟/关键帧等由 metadata 协商 + 上层插件在
      载荷内实现，解耦灵活）；StreamSender send/finish_with、StreamReceiver metadata/get_metadata/end_code/
      end_message/end_metadata；Session/Client create_stream_with_metadata；Router dispatch 走 StreamOpen 链
