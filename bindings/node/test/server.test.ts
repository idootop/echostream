...
// 运行：node dist/test/server.test.js
...
async function main(): Promise<void> {
  ...
  builder.addRpc<[string], string>("echo", async (text) => text);
  builder.addRpc<[number], { id: number; name: string }>("getUser", async (id) => ({ id, name: "alice" }));
  ...
  // RPC：字符串往返
  assert.equal(await client.request<string>("echo", "hello node"), "hello node");
  console.log("[node-client] echo 往返一致");

  // RPC：结构体载荷与响应
  const user = await client.request<{ id: number; name: string }>("getUser", 7, { decode: { id: "number", name: "string" } });
  ...
  // 客户端监听事件（验证服务端广播）
  client.onEvent<[string]>("hello", (data) => clientEvents.push(data));
  ...
  for (const s of sessions) {
    const reply = await s.request<string>("ping");
